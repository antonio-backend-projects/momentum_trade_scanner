...
def fetch_bars_multi_symbols(symbols, timeframe, start_iso, end_iso, key, sec):
    ...
    while True:
        ...
        data = resp.json()
        bars = data.get("bars", {})
        for sym, entries in bars.items():
            all_bars.setdefault(sym, []).extend(entries)
    ...
    for sym, entries in all_bars.items():
        if not entries:
            out[sym] = pd.DataFrame(columns=["t","o","h","l","c","v"])
            continue
...
def compute_signals_for_symbols(symbols, bench_map, alpaca_key, alpaca_sec,
                                ...
                                # nuovi parametri opzionali, con default “spenti”
    ...
    for sym in symbols:
        ...
        try:
            hdf = hourly.get(sym, pd.DataFrame())
            ddf = daily.get(sym, pd.DataFrame())
            bmk = bench_map.get(sym, "SPY")
            bdf = daily.get(bmk, pd.DataFrame())

            if hdf is None or ddf is None or hdf.empty or ddf.empty or bdf is None or bdf.empty:
                res["debug"]["reason"] = "missing_data"
                results[sym] = res
                continue
            ...
            allow_bar = True
            if use_1030_et:
                ny = _ny_time(last_time.to_pydatetime()); allow_bar = (ny.hour == 10 and ny.minute == 30)
            ...
            # --- PRE-SIGNAL (vicinanza al livello)
            ...
            if show_pre_signal and not long_core and not short_core:
                if not math.isnan(hh) and hh > 0:
                    dist_up_pct = abs(_pct(last_high if use_high_intrabar else last_close, hh))
            ...
            res["debug"] = {
                ...
                "dist_up_pct": None if dist_up_pct is None or math.isnan(dist_up_pct) else round(dist_up_pct, 3),
                "dist_down_pct": None if dist_down_pct is None or math.isnan(dist_down_pct) else round(dist_down_pct, 3),
            }

            res.update({
                "LONG": bool(long_core),
                "SHORT": bool(short_core),
                "pre_long": bool(pre_long),
                "pre_short": bool(pre_short),
                "rv_val": None if math.isnan(rv_val) else round(rv_val, 2),
                "ema_bias": f"L:{ema_bias_long} / S:{ema_bias_short}",
                "rs_dir": f"UP:{rs_up} / DOWN:{rs_down} (slope {round(linreg_slope(join['rs'].tail(10).values),4) if len(join['rs'].tail(10).dropna())>=2 else 0.0})",
                "trend_ok": f"L:{trend_ok_long} / S:{trend_ok_short}",
                "trigger_info": trig_info,
                "bar_time": last_time.tz_convert("UTC").isoformat(),
            })
