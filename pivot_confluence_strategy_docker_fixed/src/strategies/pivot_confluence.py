...
class PivotConfluenceStrategy:
    ...
    def on_backtest_init(self, data_map: Dict[str, pd.DataFrame]):
        for sym, df in data_map.items():
            ...
            d['date'] = d.index.date
            d['vwap'] = d.groupby('date', group_keys=False).apply(vwap)
    ...
    def _near_any_level(self, row: pd.Series, day_df: pd.DataFrame):
        ...
        if pd.notna(row.get('vwap')): candidates['VWAP'] = row['vwap']
    ...
    def on_bar_backtest(self, ts, bar_map: Dict[str, pd.Series]):
        fills = []
        near_map = {}
        day_cache = {}

        # near-level per tutti i simboli (per la confluence)
        for sym, row in bar_map.items():
            d = self.dm[sym]
            day = row.name.date()
            day_df = d[d['date'] == day]
            day_cache[sym] = day_df
        ...
        for sym in self.cfg['universe']['main']:
            ...
            # Esecuzione semplificata: prendiamo TP come uscita (proxy di 1 trade per barra)
            pnl = (tp - px)*qty if side=="buy" else (px - tp)*qty
            R = (self.params.tp_atr / self.params.stop_atr)
            fills.append([row.name, sym, side, px, tp, qty, pnl, R])
        return fills
